...
def execute_script(params)
  ...
  doc.sheets.first.rows.each do |row|
    if !row[0].nil?
      Logger.log "Creating a new plan from template 'PostFinance Release Plan' for release #{row[0]} ..."
      plan = create_plan("PostFinance Release Plan", "PostFinance #{row[0]}", Time.now)
    elsif !row[1].nil?
      ...
      request = create_request_for_plan(plan["id"], "#{stage_name} - #{sub_stage_name}", row[4], 1, app_name, env_name, false)
    elsif !row[5].nil?
      Logger.log "Creating a new step '#{row[5]}' for request '#{request["name"]}' ..."
      create_step(request["id"], row[5], "User", 1)
    end
  end
end