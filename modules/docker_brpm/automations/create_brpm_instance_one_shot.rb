...
environment = brpm_rest_client.create_environment(instance_name)

BrpmAuto.log "Linking the environment to the docker host ..."
brpm_rest_client.link_environment_to_server(environment["id"], docker_server_name)

BrpmAuto.log "Linking the environment to the app ..."
brpm_rest_client.link_environment_to_app(environment["id"], app_name)

BrpmAuto.log "Creating the installed component for Utilities ..."
brpm_rest_client.create_installed_component("Database", "", instance_name, app_name, docker_server_name)
...
brpm_rest_client.create_installed_component("DatabaseCreator", "4.3.01.06", instance_name, app_name, docker_server_name)
...
brpm_rest_client.create_installed_component("Database", "1.0.0", instance_name, app_name, docker_server_name)
...
installed_component=brpm_rest_client.create_installed_component("Application", application_version, instance_name, app_name, docker_server_name)

BrpmAuto.log "Setting the port for Application ..."
brpm_rest_client.set_property_of_installed_component(installed_component["id"], "port", port)
