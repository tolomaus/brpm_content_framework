...
def execute_script(params)
  ...
  target_environment = get_environment_by_id(params["target_environment_id"])
  ...
  target_request = create_request_from_hash(request)
  ...
  if request_params.has_key?"component_versions"
    ...
    request_params["component_versions"].each do |component_name, component_version|
      ...
      set_version_tag_of_steps_for_component(target_request, component_name, component_version)
    end
    ...
    application["components"].each do |component|
      set_version_tag_of_steps_for_component(target_request, component["name"], application_version)
    end
  end
  ...
  if params["execute_target_request"].downcase.include?("monitor")
    ...
    monitor_request(target_request["id"])
  end

  Logger.log "Adding the created request' id to the request_params ..."
  add_request_param("target_request_id", target_request["id"])
end
