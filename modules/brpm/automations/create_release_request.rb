...
# def execute_script(params)
  ...
  application = BrpmRest.get_app_by_name(P["application_name"])
  ...
  unless target_request["apps"].first["id"] == application["id"]
    ...
    # TODO workaround bug fix where the request params are not transferred to the updated application's directory
    Dir.mkdir "#{Params.automation_results_dir}/request/#{application["name"]}/#{1000 + target_request["id"].to_i}"
    json = FileUtils.mv("#{Params.automation_results_dir}/request/#{target_request["apps"].first["name"]}/#{1000 + target_request["id"].to_i}/request_data.json", "#{Params.automation_results_dir}/request/#{application["name"]}/#{1000 + target_request["id"].to_i}/request_data.json")
