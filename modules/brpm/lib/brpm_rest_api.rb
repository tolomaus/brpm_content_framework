...
def set_brpm_rest_api_url(url)
  @url = url
end

def set_brpm_rest_api_token(token)
  @token = token
end

def get_brpm_rest_api_url()
  @url ||= $params["SS_base_url"]
end

def get_brpm_rest_api_token()
  @token ||= $params["SS_api_token"]
end

def add_token(path)
  path + (path.include?("?") ? "&" : "?") + "token=#{get_brpm_rest_api_token}"
end

def brpm_get(path, options = {})
  rest_get("#{get_brpm_rest_api_url}/#{add_token(path)}", options)
end

def brpm_post(path, data, options = {})
  rest_post("#{get_brpm_rest_api_url}/#{add_token(path)}", data, options)
end

def brpm_put(path, data, options = {})
  rest_put("#{get_brpm_rest_api_url}/#{add_token(path)}", data, options)
end

def brpm_delete(path, options = {})
  rest_delete("#{get_brpm_rest_api_url}/#{add_token(path)}", options)
end
