...
class BsaSoap
  class << self
    ...
    def login
      login_with_role(get_url, get_username, get_password, get_role)
    end
    ...
    def get_all_servers(session_id, server_group)
      result = execute_cli_with_param_list(session_id, "Server", "listServersInGroup", [server_group])
      servers = get_cli_return_value(result)
    rescue => exception
      raise "Failed to get all servers for #{server_group}: #{exception.to_s}"
    end

    def validate_servers(session_id, servers = [])
      error = {}
      servers.each do |server|
        result = execute_cli_with_param_list(session_id, "Server", "printPropertyValue", [server, "AGENT_STATUS"])
    ...
    def execute_cli_with_attachments(session_id, namespace, command, args, payload)
      ...
      response = client.request(:execute_command_using_attachments) do |soap|
        soap.endpoint = "#{get_url}#{CLI_SERVICE}"
